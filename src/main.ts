...
class TunnelDock {
  ...
  constructor() {
    ...
    this.watchInterval = parseInt(process.env.TUNNELDOCK_WATCH_INTERVAL || "1000");
  }
  ...
  async syncContainer(container: CustomContainerInfo, previousState?: string): Promise<void> {
    const { shouldManage, config } = await this.dockerService.shouldManageTunnel(container, previousState);
    ...
    if (shouldManage && config) {
      ...
      try {
        ...
        // Then handle DNS
        const dnsStatus = await this.cloudflareService.manageDNSRecord(config.hostname, this.tunnelId);
        ...
        logger.error({ err: error, hostname: config.hostname }, `Error configuring tunnel`);
      }
    }
  }
  ...
  async watchContainers(): Promise<void> {
    ...
    try {
      logger.info({ watchInterval: this.watchInterval }, "Starting container and tunnel monitoring");
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.dockerService = new DockerService(this.cloudflareService.getDomain());
